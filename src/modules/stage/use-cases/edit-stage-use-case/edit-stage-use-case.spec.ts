...
describe("Edit Project Use Case", () => {
  ...
  it("Should edit a project successfully", async () => {
    ...
    const projectData = makeProject({ name: "Initial Name", description: "Initial Desc", userId: "user-123" });
  ...
  it("Should throw ForbiddenException if user does not own the project", async () => {
    ...
    ).rejects.toThrow(new ForbiddenException("You do not have permission to edit this project"));
  });
});
